...
from model_layers import ModelLayer, ModelLayersRegistry, LAYER_KIND_PROMPT
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        registry.add_layer(
            ...
            ModelLayer(
                ...
                description="Кодовый адаптер",
                size_gb=0.6,
                adapter="code-assistant-lora.bin",
            ),
            activate=True,
        )
        registry.save()
        ...
        assert_equal(loaded.get_active_adapter(base), "code-assistant-lora.bin", "Активный адаптер должен сохраниться")
...
def run_prompt_layer() -> None:
    with tempfile.TemporaryDirectory() as tmp_dir:
        cfg_path = Path(tmp_dir) / "model_layers.json"
        registry = ModelLayersRegistry(cfg_path)

        base = "neira-cell-router:latest"
        registry.add_layer(
            base,
            ModelLayer(
                id="cells-prompt",
                kind=LAYER_KIND_PROMPT,
                description="Промпт для работы с клетками",
                system_prompt="Всегда упоминай клетки и органы Neira.",
            ),
            activate=True,
        )
        registry.save()
        ...
        loaded = ModelLayersRegistry(cfg_path)
        assert_equal(
            loaded.get_active_prompt(base),
            "Всегда упоминай клетки и органы Neira.",
            "Активный промпт должен сохраниться",
        )

        loaded.update_layer(base, "cells-prompt", system_prompt="Обновлённый промпт.")
        loaded.save()

        loaded2 = ModelLayersRegistry(cfg_path)
        assert_equal(
            loaded2.get_active_prompt(base),
            "Обновлённый промпт.",
            "Промпт должен обновиться",
        )
