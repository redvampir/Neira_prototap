# 🌐 Neira Multi-Provider LLM System

## 🎯 Теперь Neira независима от Ollama!

Вместо зависимости от одного провайдера, Neira может переключаться между:
- **mistral.rs** (локально, основа — Rust runtime)
- **LM Studio** (локально, GUI)
- **Ollama** (локально, бесплатно, приватно)
- **Groq** (облако, бесплатно, очень быстро)
- **OpenAI** (GPT-3.5/4, качество, недорого)
- **Claude** (Anthropic, лучшее качество, дороже)

**Gemini** в коде пока не реализован (нет провайдера в `llm_providers.py`).

## ⚡ Быстрый старт

### 1. Получи API ключи (если используешь облако)

#### Groq (РЕКОМЕНДУЕТСЯ - быстро и бесплатно!)
```
1. Зайди: https://console.groq.com/keys
2. Создай аккаунт (GitHub/Google)
3. Скопируй API key: gsk_...
```

#### OpenAI (если нужен GPT-4)
```
1. Зайди: https://platform.openai.com/api-keys
2. Добавь карту ($5 минимум)
3. Создай ключ: sk-...
```

#### Claude (лучшее качество)
```
1. Зайди: https://console.anthropic.com/settings/keys
2. Добавь карту ($5 минимум)
3. Создай ключ: sk-ant-...
```

### 2. Настрой .env файл

```bash
# Скопируй пример
cp .env.example .env

# Открой и вставь свои ключи
notepad .env
```

Пример `.env`:
```env
# Бесплатно и быстро!
GROQ_API_KEY=gsk_ваш_ключ

# Опционально (платно)
OPENAI_API_KEY=sk-ваш_ключ
ANTHROPIC_API_KEY=sk-ant-ваш_ключ

# Режим работы: free / balanced / quality
NEIRA_MODE=balanced

# Приоритет провайдеров (первый пробуется первым)
LLM_PROVIDER_PRIORITY=mistralrs,ollama,groq,openai
```

Если `LLM_PROVIDER_PRIORITY` не задан, приоритет берётся из
`llm_providers._DEFAULT_PROVIDER_PRIORITY` (см. код).

### 3. Запусти Neira

```bash
python telegram_bot.py
```

**Теперь если mistral.rs недоступен, Neira автоматически переключится на Groq!**

## 🎨 Режимы работы

### FREE (только бесплатные)
```env
NEIRA_MODE=free
```
- mistral.rs (локально)
- Ollama (локально)
- Groq (бесплатно в облаке)
- **Стоимость: $0**

### BALANCED (баланс цены и качества)
```env
NEIRA_MODE=balanced
```
- mistral.rs → Ollama → Groq → GPT-3.5-turbo
- **Стоимость: ~$0.002/1000 токенов**

### QUALITY (максимум качества)
```env
NEIRA_MODE=quality
```
- Claude Sonnet → GPT-4 → Groq → Ollama → mistral.rs
- **Стоимость: ~$0.03/1000 токенов**

## 🔄 Как работает автоматический fallback

```
Пользователь: "Привет!"
    ↓
1. Пробуем mistral.rs (локально)
   ❌ Ошибка: "memory layout cannot be allocated"
    ↓
2. Пробуем Groq (llama-3.1-8b-instant)
   ✅ Успех! Ответ за 0.5 секунды
    ↓
Ответ пользователю
```

## 📊 Сравнение провайдеров

| Провайдер | Скорость | Качество | Стоимость | Приватность |
|-----------|----------|----------|-----------|-------------|
| **mistral.rs** | ⚡⚡ Быстро | ⭐⭐⭐⭐ | 🆓 Бесплатно | 🔒 100% |
| **LM Studio** | ⚡⚡ Быстро | ⭐⭐⭐⭐ | 🆓 Бесплатно | 🔒 100% |
| **Ollama** | 🐌 Медленно | ⭐⭐⭐ | 🆓 Бесплатно | 🔒 100% |
| **Groq** | ⚡⚡⚡ ОЧЕНЬ быстро | ⭐⭐⭐⭐ | 🆓 Бесплатно | ⚠️ В облаке |
| **OpenAI** | ⚡⚡ Быстро | ⭐⭐⭐⭐⭐ | 💰 Дешево | ⚠️ В облаке |
| **Claude** | ⚡⚡ Быстро | ⭐⭐⭐⭐⭐⭐ | 💰💰 Средне | ⚠️ В облаке |

## 🛠️ Продвинутая настройка

### Выбор модели для локального рантайма

Сейчас рантайм использует **одну модель на провайдера**, а per-task модели
(code/reason/personality) не применяются в рабочем цикле. Если вам важно менять
модель — используйте переменные ниже. Они реально читаются рантаймом.

```env
# mistral.rs
NEIRA_MISTRALRS_MODEL=default

# Ollama
NEIRA_OLLAMA_MODEL=qwen2.5:3b

# LM Studio
NEIRA_LMSTUDIO_MODEL=qwen/qwen2.5-coder-14b
```

> Примечание: `LLM_CODE_MODEL_*`, `LLM_REASON_MODEL_*`, `LLM_PERSONALITY_MODEL_*`
> сейчас используются только для вывода конфигурации в `neira_config.py`, но не
> влияют на выбор модели в рантайме.

### Кастомный приоритет провайдеров

**Фактический дефолт приоритета (если переменная не задана):**
1. Mistral.rs
2. LM Studio
3. Ollama
4. llama.cpp server
5. Groq
6. OpenAI
7. Claude

```env
# Только mistral.rs
LLM_PROVIDER_PRIORITY=mistralrs

# Сначала качество, потом скорость
LLM_PROVIDER_PRIORITY=claude,openai,groq,ollama,mistralrs
```

### Условия переключения на облако

```env
# Использовать облако если локальный провайдер недоступен
USE_CLOUD_IF_OLLAMA_FAILS=true

# Использовать облако если сложность задачи > 4
USE_CLOUD_IF_COMPLEXITY=4

# Использовать облако после 2 неудачных попыток
USE_CLOUD_IF_RETRIES=2
```

## 💡 Примеры использования

### Минимальная конфигурация (только mistral.rs)

```env
NEIRA_MODE=free
LLM_PROVIDER_PRIORITY=mistralrs
```

### Максимальная надёжность (все провайдеры)

```env
GROQ_API_KEY=gsk_...
OPENAI_API_KEY=sk-...
ANTHROPIC_API_KEY=sk-ant-...

NEIRA_MODE=balanced
LLM_PROVIDER_PRIORITY=mistralrs,ollama,groq,openai,claude
```

### Только облако (без локальных моделей)

```env
GROQ_API_KEY=gsk_...
OPENAI_API_KEY=sk-...

NEIRA_MODE=quality
LLM_PROVIDER_PRIORITY=mistralrs,groq,openai
```

## 📈 Проверка конфигурации

```bash
python neira_config.py
```

Вывод:
```
==================================================
🧠 NEIRA CONFIGURATION
==================================================

📋 Режим: BALANCED

🔑 API Keys:
  ✓ Groq
  ✓ OpenAI
  ✗ Claude (Anthropic)
  ✗ Gemini

🎯 Приоритет провайдеров: mistralrs → ollama → groq → openai

🤖 Модели:
  code: nemotron-mini
  reason: nemotron-mini
  personality: gpt-3.5-turbo

⚙️ Настройки:
  Ollama timeout: 180s
  Cloud timeout: 60s
  Max retries: 2
  Min score: 7/10
==================================================
```

## 🐛 Решение проблем

### Neira не отвечает

**Проверь логи:**
```
INFO:root:Trying mistralrs (default)...
WARNING:root:✗ Failed with mistralrs: memory layout cannot be allocated
INFO:root:Trying groq (llama-3.1-8b-instant)...
INFO:root:✓ Success with groq
```

**Если все провайдеры fail:**
1. Проверь API ключи в `.env`
2. Проверь интернет-соединение
3. Проверь лимиты API ключей

### Groq API ошибка

```
HTTP 429: Rate limit exceeded
```

**Решение:** Добавь `OPENAI_API_KEY` как fallback

### Слишком дорого

**Настрой лимиты в коде:**
```python
# В llm_providers.py
class LLMManager:
    def __init__(self, max_cost_per_request=0.01):
        self.max_cost = max_cost_per_request
```

## 📚 API Reference

### Создание менеджера

```python
from llm_providers import LLMManager, create_default_manager

# Дефолтный (все провайдеры)
manager = create_default_manager()

# Только быстрые
from llm_providers import create_fast_manager
manager = create_fast_manager()

# Только качественные
from llm_providers import create_quality_manager
manager = create_quality_manager()

# Кастомный
from llm_providers import OllamaProvider, GroqProvider
manager = LLMManager([
    GroqProvider(model="llama-3.1-70b-versatile"),
    OllamaProvider(model="qwen2.5:0.5b")
])
```

### Генерация ответа

```python
response = manager.generate(
    prompt="Привет!",
    system_prompt="Ты - Нейра",
    temperature=0.7,
    preferred_provider=ProviderType.GROQ  # Опционально
)

if response.success:
    print(response.content)
    print(f"Provider: {response.provider.value}")
    print(f"Cost: ${response.cost:.4f}")
else:
    print(f"Error: {response.error}")
```

## 🎓 Дальнейшее чтение

- [Groq Documentation](https://console.groq.com/docs)
- [OpenAI API Reference](https://platform.openai.com/docs/api-reference)
- [Claude API Reference](https://docs.anthropic.com/en/api)

## ✅ Чеклист миграции

- [ ] Получил Groq API key (бесплатно!)
- [ ] Создал `.env` файл
- [ ] Указал `GROQ_API_KEY` в `.env`
- [ ] Запустил `python neira_config.py` для проверки
- [ ] Перезапустил Telegram бота
- [ ] Отправил тестовое сообщение
- [ ] Neira ответила (проверь в логах какой провайдер использован)

**Готово! Теперь Neira работает даже без Ollama 🎉**
